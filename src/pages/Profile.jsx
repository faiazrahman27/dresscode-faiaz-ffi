  ...
  if (block.type === 'avatar') {
    ...
    return (
      <div className={`${rowClass} ${alignClass}`}>
        <div className="h-24 w-24 overflow-hidden rounded-full border border-[rgba(94,207,207,0.18)] bg-[rgba(255,255,255,0.03)]">
          {block.imageUrl ? (
            <img
              src={block.imageUrl}
...
export default function Profile() {
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#0A1F1F] text-white">
        Loading profile...
      </div>
    )
  }
  ...
  if (state === 'not-found') {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#0A1F1F] px-4 text-white">
  ...
  return (
    ...
    >
      <div className="container max-w-6xl">
        {isOwnerViewing ? (
          <div className="mb-6 flex flex-wrap justify-end gap-3">
            <button
              type="button"
              onClick={() => navigate(-1)}
              className="rounded-full border border-[rgba(94,207,207,0.12)] bg-[rgba(255,255,255,0.03)] px-4 py-2 text-sm text-white/78 transition hover:text-white"
            >
              Back
            </button>
            ...
            <button
              ...
              className="rounded-full bg-[#5ECFCF] px-4 py-2 text-sm font-medium text-[#071515] transition hover:opacity-90"
            >
              Dashboard
            </button>
          </div>
        ...
        {navbar.enabled ? (
          <div className="sticky top-4 z-40 mb-6 rounded-[20px] border border-[rgba(94,207,207,0.14)] bg-[rgba(8,24,24,0.78)] px-4 py-4 backdrop-blur-xl">
        ...
        <div className="mb-8 grid gap-6 xl:grid-cols-[1fr_auto] xl:items-start">
          <div>
            ...
            {isOpen ? (
              <div className="max-w-2xl rounded-[18px] border border-emerald-500/20 bg-emerald-500/10 p-4 text-sm text-white/80">
                This is an open code. Its owner can personalize the live page after activation.
              </div>
            ) : null}

            {isLocked ? (
              <div className="max-w-2xl rounded-[18px] border border-[rgba(94,207,207,0.16)] bg-[rgba(94,207,207,0.08)] p-4 text-sm text-white/80">
                ...
                {(section.columns || []).map((column) => (
                  <div key={column.id} className="grid gap-4">
                    {(column.blocks || []).map((block) => (
                      <div key={block.id}>{renderBlock(block, accentColor)}</div>
                    ))}
                  </div>
                ))}
              </div>
